...
utils.buildSchema = function(obj, foreignKeys) {
  ...
  Object.keys(obj).forEach(function(key) {
    ...
    const realType = /TEXT|INTEGER|REAL|BLOB/i.exec(sqliteType)[0];
...
utils.buildSelectStatement = function(criteria, table, schema) {
  var query = '';
  ...
  /*************************************************
   * Commenting out below because new waterline
   * deals with aggregates in separate functions.
   * However, I am keeping the code for reference
   * as I migrate it into those other functions
   *************************************************/
  ...
  if (criteria.select && criteria.select.length >>> 0 > 0) {
    for (let key of criteria.select) {
      selectKeys.push({ table: tableName, key });
    }
  } else {
    for (let columnName in schema) {
      selectKeys.push({ table: tableName, key: columnName });
    }
  }

  // Check for joins
  // NOTE: keeping joins here until I figure out how the
  // new Waterline join method works
  if (criteria.joins) {
  ...
  selectKeys.forEach(function(select) {
    query += select.table + '.'
      + utils.escapeName(select.key) + ', ';
  });

  // Add all the columns from the joined tables
  // NOTE keeping joins here until I know how the new waterline works
