...
utils.buildSchema = function(obj, foreignKeys) {
  ...
  Object.keys(obj).forEach(function(key) {
    ...
    //remove autoincrement from pk defs
    attr.autoIncrement = attr.primaryKey ? false : attr.autoIncrement;

    const def = [
      '"' + key + '"',                        // attribute name
      utils.sqlTypeCast(attr.columnType, key),     // attribute type
      attr.primaryKey ? 'PRIMARY KEY' : '',   // primary key
      attr.unique ? 'UNIQUE' : '',             // unique constraint
      attr.autoIncrement ? 'AUTOINCREMENT' : '' //autoincrement
    ].join(' ').trim();
  ...
  return columnDefs.concat(constraintDefs).join(', ');
};
