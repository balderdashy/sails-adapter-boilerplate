...
var adapter = module.exports = (function() {
  ...
  return {
    ...
    create: function(collectionName, values, cb) {
      ...
      var query = [
        'CREATE (n:' + collectionName + ' { ' + parseOne(values) + ' })',
        'RETURN n'
      ];
    ...
    createMany: function(collectionName, map, cb) {
      // Create a single new model specified by values
      var query = [
        'CREATE (n:' + collectionName + ' { ' + parseMany(map) + ' })',
        'RETURN n'
      ];

      neo.graph(function(gr) {
        gr.query(query.join('\n'), map, function (err, results) {
      // Respond with error or newly created model instance
          if (err) {
            console.log(err);
            console.log(err.stack);
    ...
    find: function(collectionName, options, cb) {
      ...
      var query = [
        'MATCH (n)',
        'WHERE n:' + collectionName + ' AND ' + toWhere('n', options),
        'RETURN n'
      ];
