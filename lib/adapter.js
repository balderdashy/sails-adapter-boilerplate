...
var adapter = module.exports = (function() {
  ...
  function andJoin(object, properties) {
    var query = [], q;
  ...
  function toWhere(object, params) {
    var properties = params.where;
    if (!properties)
      return "";
    var query = [], q;
    var count = 0;
    ...
    {
      var targetProperties = new Object();
      for (var i in properties["or"])
      {
        q = andJoin(object, properties["or"][i]);
        query.push(q);
      ...
      query.push(andJoin(object, properties));
  ...
  function query(q, params, cb) {
    ...
    // Respond with error or newly created model instance
        if (err) {
          ...
          {
            var data = _.clone(results[i].n.data);
            var id = _.pick(results[i].n, 'id');
  ...
  function createQuery(q, params, cb) {
    neo.graph(function(gr) {
      if (defaults.debug) {
        console.log('q: ', q.join('\n'), 'p: ', params); // spit out the queries for debug
      }
