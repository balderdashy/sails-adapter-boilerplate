...
const Query = function(schema, models) {
  this._values = [];
  this._paramCount = 1;
  this._query = '';
  /** Waterline models - provides info on type */
  this._models = models || {};
...
Query.prototype.castRow = function(tableName, values) {
  const waterlineModel = this._models[tableName];
  const newModel = {};

  for (let columnName of values) {
    const attrDef = waterlineModel.definition[columnName];
