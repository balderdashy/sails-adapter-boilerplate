...
class Criterion {
  ...
  generateCriteria(paramIndex = -1) {
    ...
    if (Object.keys(this._rawCriterion).length > 1) {
      throw new Error('Unexpected query object: more than one column key');
    }
    for (let columnName in this._rawCriterion) {
      // get schema type so we know how to process
      const columnType = this._schema[columnName];

      /************************************
       * START HERE
       * Parse operators
       * Do checks for operators with valid
       * data types
       ***********************************/
    }
  }
